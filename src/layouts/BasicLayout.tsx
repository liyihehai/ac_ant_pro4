...
const IconFont = createFromIconfontCN({
    scriptUrl: '//at.alicdn.com/t/font_2019524_z36pjvjsrlp.js',
});

const getIcon = (icon?: string | React.ReactNode): React.ReactNode => {
  if (icon!=null && typeof icon === 'string' && icon.startsWith('icon-')) {
...
const BasicLayout: React.FC<BasicLayoutProps> = (props) => {
  ...
  useEffect(() => {
    if (dispatch) {
      dispatch({
        ...
        callback:(data:MenuDataItem|any)=>{
  ...
  //     "children": [
  //       {
  //         "path": "/autocode/project",
  //         "name": "项目管理",
  //         "icon": "icon-deleteteam",
  //       },
  //       {
  //         "path": "/autocode/connect",
  //         "name": "连接管理",
  //         "icon": "icon-deleteteam",
  //         "children":[
  //           {
  //             "name":'连接列表',
  //             "path": '/autocode/connect/conlist',
  //             "icon": "icon-deleteteam",
  //           }
  //         ]
  //       }
  ...
  //useEffect(() => { setMenuData(data);}, []);

  const mappingIcon = (menuData:MenuDataItem[]):MenuDataItem[] => {
    const mappingMenu = menuData.map((item:MenuDataItem) => ({
      ...item,
  ...
  const iconMenuData = mappingIcon(userMenuData);
    ...
    <>
      <ProLayout
        ...
        subMenuItemRender={(HeaderViewProps, defaultDom) => {
          return (
            <div>
              {HeaderViewProps.icon}{HeaderViewProps.name}
            </div>
          );
        }}
